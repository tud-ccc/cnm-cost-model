...
from kernels.Scan import TestOp
...
_OUTPUT_FILE = Path(__file__).resolve().parent / "output" / "scan_comparison.csv"
...
def _predict_row(row):
    ...
    # scan.csv's "iteration" column is the sweep's chunk count, not
    # ITER_PER_THREAD -- matches ReduceOp/Sel's run_benchmark.py convention
    # (iteration*buffer_size passed as MAP_ITER_DIMS[0]).
