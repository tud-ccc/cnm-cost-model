...
from configuration import reduceop_file
from kernels.ReduceOp_xdsl import TestOp
...
# Path B of the comparison started for VectorOp: same real hardware
# measurements (reduceop_file), same Simulator, but predicted from
# kernels/ReduceOp_xdsl.py's hand-transcribed CnmGen/output/llvcnm/reduceop.llvcnm
# instruction list instead of ReduceOp.py's hand-transcribed real assembly.
_OUTPUT_FILE = Path(__file__).resolve().parent / "output" / "reduceop_comparison_xdsl.csv"
...
def main():
    with open(reduceop_file, newline="") as result_file:
