...
from kernels.ReduceOp import TestOp
...
_OUTPUT_FILE = Path(__file__).resolve().parent / "output" / "reduceop_comparison.csv"
...
def _predict_row(row):
    ...
    # reduceop.csv's "iteration" column is the sweep's chunk count, not
    # ITER_PER_THREAD -- run_benchmark.py passes iteration*buffer_size as
    # MAP_ITER_DIMS[0] (see ReduceOp/run_script/run_benchmark.py).
