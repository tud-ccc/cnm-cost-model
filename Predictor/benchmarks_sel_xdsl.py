...
from configuration import sel_file
from kernels.Sel_xdsl import TestOp
...
# Path B of the comparison started for VectorOp/ReduceOp/Scan/Hst: same real
# hardware measurements (sel_file), same Simulator, but predicted from
# kernels/Sel_xdsl.py's hand-transcribed CnmGen/output/llvcnm/sel.llvcnm
# instruction list instead of Sel.py's hand-transcribed real assembly.
_OUTPUT_FILE = Path(__file__).resolve().parent / "output" / "sel_comparison_xdsl.csv"
...
def main():
    with open(sel_file, newline="") as result_file:
