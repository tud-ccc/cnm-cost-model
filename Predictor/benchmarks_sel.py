...
from kernels.Sel import TestOp
...
_OUTPUT_FILE = Path(__file__).resolve().parent / "output" / "sel_comparison.csv"
...
def _predict_row(row):
    ...
    # sel.csv's "iteration" column is the sweep's chunk count, not
    # ITER_PER_THREAD -- run_benchmark.py passes iteration*buffer_size as
    # MAP_ITER_DIMS[0] (see Sel/run_script/run_benchmark.py).
